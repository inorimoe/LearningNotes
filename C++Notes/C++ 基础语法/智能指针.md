- [智能指针](#智能指针)
  ...
  - [注意事项 / 实战避坑](#注意事项--实战避坑)
    - [1、使用 make\_shared/make\_uniqe 构建智能指针](#1使用-make_sharedmake_uniqe-构建智能指针)
    - [2、不要使用类的this指针构建shared\_ptr](#2不要使用类的this指针构建shared_ptr)
      - [数据成员 shared\_ptr：](#数据成员-shared_ptr)
      - [函数成员返回shared\_ptr](#函数成员返回shared_ptr)
    - [3、使用 std::enable\_shared\_from\_this 共享该对象的所有权](#3使用-stdenable_shared_from_this-共享该对象的所有权)
...
## 注意事项 / 实战避坑
### 1、使用 make_shared/make_uniqe 构建智能指针
...
```
用裸指针构建智能指针不优雅，又可能出现问题，复杂逻辑情况下可能难以发现错误。
使用make_shared/make_uniqe 构建智能指针，保证出现的指针类型一致，而且风格统一。
### 2、不要使用类的this指针构建shared_ptr
#### 数据成员 shared_ptr：
...
### 3、使用 std::enable_shared_from_this 共享该对象的所有权
https://www.bilibili.com/video/BV1du4y1w7Mg
https://mp.weixin.qq.com/s?__biz=MzI2NjYwOTAyMg==&mid=2247488473&idx=1&sn=ba4d5e106d829047e35508957d3176a9&chksm=ea8adbc8ddfd52de92dcc53f309dab2bc9ccf7091d98129e7867b808f1283a96efa7c85c1ece&token=73524073&lang=zh_CN#rd
