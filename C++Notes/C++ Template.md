...
C++标准目前不支持函数模板偏特化，函数允许重载，声明另一个函数模板即可替代偏特化的需要：（C++禁止的原因是在于函数模板偏特化和函数重载决策的矛盾）

全特化：限定模板实现的具体类型, 使用template<>;[<u>参考: 函数模板的特化</u>](https://blog.csdn.net/ljq32/article/details/7916723)
偏特化：提供另一份template定义式，而其本身仍为templatized，这是针对于template参数更进一步的条件限制所设计出来的一个特化版本。也就是如果这个模板有多个类型，那么**只限定其中的一部分**;

函数模板偏特化有通用方案[<u>参考:C++函数模板的偏特化</u>](https://zhuanlan.zhihu.com/p/268600376);
